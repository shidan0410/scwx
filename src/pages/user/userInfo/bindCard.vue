<template>
  <div class="verifyCard">
    <div class="input_group">
		<input type="text" id="realname" v-model="realname" @blur="verify_name()" maxlength="32" placeholder="请输入姓名字">
		<span class="icon_user"></span>
		<span class="errorMsg" v-if="realnameError">{{realnameError}}</span>
    <div style="clear:both"></div>
	</div>
	<div class="input_group">
		<input type="text" id="idcard" v-model="idcard" @keyup="" placeholder="请输入身份证号">
  ...
  <div class="banklist" v-show="banklistShow">
	...
	<div class="input_group">
		<input type="tel" id="bancktel" v-model="bancktel" @keyup="" maxlength="11" placeholder="请输入银行卡预留手机号">
	...
	<div class="form-group">
        ...
        <input type="number" id="verifycode" v-model="verifycode" @focus="verify_tel()" @keyup="verify_code()" placeholder="请输入手机验证码">
        ...
        registNextConfirm(this.verifycode,Encrypt(this.buypwd)).then(res=>{
            if(res.code=="000"){
               if(res.isSign == '1'){//需要网银签约
                    this.$router.push({path:'/bankContract?product='+productId+'&bankCd='+res.bankId,query:{title:'网银签约'}})
               }else{
                  this.$router.push({path:'/risk?isShow=true&from=verifyCard',query:{title:'风险评测'}})
               }
