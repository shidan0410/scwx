<template>
  <div class="search_wrapper">
    ...
    <mt-loadmore class="needsClick" :bottom-method="loadBottom" :bottom-all-loaded="allLoaded" ref="loadmore" @bottom-status-change="handleBottomChange" :auto-fill="false">
      <user-tenders v-bind:userTenders="userTenders"></user-tenders>
    ...
    <div v-show="historytitle" class="search_history">
      ...
      <ul class="search_ul">
        <li v-for="h in hisArray" @click="search(h)">
...
<script type="text/ecmascript-6">
  ...
  import {getMyAppTenderList} from '@/service/getData'
  ...
  export default {
    ...
    mounted() {
      ...
      if(localStorage.getItem('hisArray')){
        this.hisArray = localStorage.getItem('hisArray').split(',');
        this.clearFlag = true;
      }else{
    ...
    methods:{
      ...
      search(proName){
        if(proName!="暂无搜索历史"){
          ...
          getMyAppTenderList(this.curPage,proName).then(res => {
              this.historytitle = false;//将搜索记录隐藏
              if(res.userTenders.length>0){
                this.totalGetCount = res.MyTenderListCnt;
              ...
              //保存搜索历史
              var hisArray = [];
              if(localStorage.getItem('hisArray'))hisArray=localStorage.getItem('hisArray').split(",");//获取已有的历史，变成数组格式
              if(this.inputVaule.length>0){
                if(hisArray.indexOf(this.inputVaule)==-1)hisArray.push(this.inputVaule);//将新纪录添加进去
                localStorage.setItem('hisArray',hisArray);
              }
      ...
      loadBottom(){//请求后台
         this.allLoaded = false;
          //传入记录类型 和 要请求的页码
          this.curPage +=1;
        getMyAppTenderList(this.curPage,this.inputVaule).then(res => {
                if(res.userTenders.length == 0){
